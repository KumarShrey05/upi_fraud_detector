...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((open) => !open) : setOpen((open) => !open)}, [isMobile, setOpen, setOpenMobile])
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('keydown', handleKeyDown)}, [toggleSidebar])
...
function SidebarMenuButton({
  ...
  asChild?: boolean
  isActive?: boolean
  tooltip?: string | React.ComponentProps<typeof TooltipContent>} & VariantProps<typeof sidebarMenuButtonVariants>) {
  const Comp = asChild ? Slot : 'button'
